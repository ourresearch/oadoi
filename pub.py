...
from pub_page import get_pages, publisher_equivalent
...
class Pub(db.Model):
    ...
    def ask_green_locations(self):
        ...
        for my_page in [p for p in self.pages if p.pmh_id != publisher_equivalent]:
            if my_page.is_open:
    ...
    def mint_pages(self):
        for p in get_pages(self):
            logger.info(u'minted page: {}'.format(p))
            db.session.merge(p)
