    ...
    def to_dict(self):
        ...
        if self.name in configs:
            ...
            try:
                ...
                description = configs[self.name]["description"]
    ...
    def decide_if_assigned(self, person):
        ...
        for my_product in person.products:
            if my_product.altmetric_score > 250:
                self.assigned = True
                self.level = 3
                products[3].append(my_product)
            elif not self.assigned and my_product.altmetric_score > 100:
                self.assigned = True
                self.level = 2
                products[2].append(my_product)
            elif not self.assigned and my_product.altmetric_score > 50:
                self.assigned = True
                self.level = 1
                products[1].append(my_product)
        if self.assigned:
            self.candidate_badge.add_products(products[self.level])
...
#     is_for_products = False
#     group = "poster_types"
#     description = {1: "More than 10% of your impact is from science communicators."}
#
#     def decide_if_assigned(self, person):
